...
/* Class  Control : jabatan  * di Buat oleh Diar PHP Generator * Update List untuk grid karena program generatorku lom sempurna ya hehehehehe */
...
//  $this->load->view('test.php');
    ...
    function createform($xidx, $xAwal=0, $xSearch='') {
        ...
        $xForm = '<div id="stylized" class="myform"><h3> Isi Edit Pejabat</h3>' . form_open_multipart('ctrjabatan/inserttable', array('id' => 'form', 'name' => 'form')).'<div class="garis"></div>';
        ...
        echo $this->modelgetmenu->SetViewPerpus($xForm . $this->setDetailFormjabatan($xidx), $this->getlistjabatan($xAwal, $xSearch), '', $xAddJs, '');
    }

    function setDetailFormjabatan($xidx) {
        $this->load->helper('form');
        $this->load->model('modeljabatan');
        $row = $this->modeljabatan->getDetailjabatan($xidx);
        $xBufResult = '';
        if (empty($row)) {
            $xidx = '0';
            $xNmJabatan = '';
            ...
            $xidx = $row->idx;
            $xNmJabatan = $row->NmJabatan;
        }
        $this->load->helper('common');
        $xBufResult = '<input type="hidden" name="edidx" id="edidx" value="0" />' .
                $xBufResult .= setForm('edNmJabatan', 'Nama Jabatan', form_input(getArrayObj('edNmJabatan', $xNmJabatan, '200'))) . '<div class="spacer"></div>';
    ...
    function inserttable() {
        $xidx = $_POST['edidx'];
        $xNmJabatan = $_POST['edNmJabatan'];

        $this->load->model('modeljabatan');
        if ($xidx != '0') {
            $this->modeljabatan->setUpdatejabatan($xidx, $xNmJabatan);
        } else {
            $this->modeljabatan->setInsertjabatan($xidx, $xNmJabatan);
        }
        $this->createform('0');
    }
    ...
    function getlistjabatan($xAwal, $xSearch) {
        ...
        $xbufResult = addRow(addCell('idx', 'width:100px;', true) .
                        addCell('NmJabatan', 'width:200px;', true) .
        ...
        foreach ($xQuery->result() as $row) {
            ...
            $xButtonHapus = '<img src="' . base_url() . 'resource/imgbtn/delete_table.png" alt="Hapus Data" onclick = "dohapus(\'' . $row->idx . '\',\'' . substr($row->NmJabatan, 0, 20) . '\');" style="border:none;">';
            $xbufResult .= addRow(addCell($row->idx, 'width:100px;') .
                            addCell($row->NmJabatan, 'width:200px;') .
    ...
    function actionrecord($xIdRec='', $xAction='') {
        ...
        switch ($xAction) {
            case 'edit':
                ...
                break;
            case 'hapus':
                $this->modeljabatan->setDeletejabatan($xIdRec);
                $this->createform('0');
                break;
            case 'search' :
    ...
    function editrec() {
        ...
        $this->json_data['NmJabatan'] = $row->NmJabatan;
    ...
    function search() {
        ...
        $this->json_data['tabledata'] = $this->getlistjabatan($xAwal, $xSearch);
        echo json_encode($this->json_data);
    }
    ...
    function simpan() {
        ...
        $this->load->model('modeljabatan');
        if ($xidx != '0') {
            $xStr = $this->modeljabatan->setUpdatejabatan($xidx, $xNmJabatan);
        } else {
            $xStr = $this->modeljabatan->setInsertjabatan($xidx, $xNmJabatan);
        }
    }
