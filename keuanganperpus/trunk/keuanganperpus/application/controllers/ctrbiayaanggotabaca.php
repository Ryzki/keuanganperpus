...
class ctrbiayaanggotabaca extends CI_Controller {
...
//  $this->load->view('test.php');
    ...
    function createform($xidx, $xAwal=0, $xSearch='') {
        ...
        $xForm = '<div id="stylized" class="myform"><h3>Setting Biaya Anggota Baca a</h3>' . form_open_multipart('ctrhargajenistransaksi/inserttable', array('id' => 'form', 'name' => 'form')).'<div class="garis"></div>';
        $xAddJs = '<script language="javascript" type="text/javascript" src="' . base_url() . 'resource/js/tiny_mce/jquery.tinymce.js"></script>' .
                ...
                '<script language="javascript" type="text/javascript" src="' . base_url() . 'resource/ajax/ajaxbiayaanggotabaca.js"></script>' .
        ...
        //echo $this->modelgetmenu->SetViewPerpus($xForm . $this->setDetailFormhargajenistransaksi($xidx), $this->getlisthargajenistransaksi($xAwal, $xSearch), 'login', $xAddJs, '');
        echo $this->modelgetmenu->SetViewPerpus($xForm . $this->setDetailFormhargajenistransaksi($xidx), '', '', $xAddJs, '');
    }
    ...
    function setDetailFormhargajenistransaksi($xidx) {
        ...
        $xBufResult = '<input type="hidden" name="edidx" id="edidx" value="1" />' .
        $xBufResult .= '<input type="hidden" name="idJenisTransaksi" id="idJenisTransaksi" value="2" />';
        //$xBufResult .= setForm('edidJenisTransaksi', 'idJenisTransaksi', form_input(getArrayObj('edidJenisTransaksi', '2', '100'),'type="hiden"')) . '<div class="spacer"></div>';
        $xBufResult .= setForm('edbiaya', 'Biaya Anggota Baru', form_input(getArrayObj('edbiaya', $xbiaya, '100'))) . '<div class="spacer"></div>';
    ...
    function simpan() {
        ...
        $this->load->model('modelhargajenistransaksi');
        if ($xidx != '0') {
            $xStr = $this->modelhargajenistransaksi->setUpdatehargajenistransaksi($xidx, '2', $xbiaya);
        } else {
            $xStr = $this->modelhargajenistransaksi->setInserthargajenistransaksi($xidx, '2', $xbiaya);
        }
    }
