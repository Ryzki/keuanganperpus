...
//  $this->load->view('test.php');
    ...
    function getArrayTunaiNonTunai(){
      $xBuffResul =null;
      $xBuffResul[0] = 'Tunai';
      $xBuffResul[1] = 'Non Tunai';
    return $xBuffResul;
    }
    ...
    function setDetailFormReport($xidx) {
        ...
        $xBufResult .= setForm('edtunainontunai', 'Tunai/Non Tunai', form_dropdown('edtunainontunai', $this->getArrayTunaiNonTunai(), '0', 'id="edtunainontunai" width="150px"')) . '<div class="spacer"></div>';
        ...
        $xBufResult .= '<div class="garis"></div>' . form_button('btSimpan', 'Tampil Data', 'onclick="dotampillaporanfotokopibulantunainontunai(false);"') . form_button('btNew', 'Export Ke Excel', 'onclick="dotampillaporanfotokopibulantunainontunai(true);"') . '<div class="spacer"></div>';
        return $xBufResult;
    }
    ...
    function  dotampillaporan(){
     ...
     $xTunaiNonTunai= $_POST['edtunainontunai'];
     $edidlokasi = $_POST['edidlokasi'];
     $this->load->helper('json');
     $this->json_data['data'] =$this->getReport($xBulan,$xTahun,$edidlokasi,$xTunaiNonTunai);
     echo json_encode($this->json_data);
    }

    function getReport($xBulan,$tahun,$xidlokasi,$xTunaiNonTunai) {
        //$xIdEdit = $_POST['edidx'];
        $this->load->helper('form');
        $this->load->helper('common');

        $arrayrow = $this->getrow($xBulan,$tahun,$xidlokasi,$xTunaiNonTunai);
       ...
       $nmbulan =  $array[str_pad($xBulan, 2, '0',STR_PAD_LEFT)];
        ...
        if($xTunaiNonTunai=='0')
        {$xTunaiNonTunai = 'TUNAI';}
        else
        {$xTunaiNonTunai = 'NON TUNAI';}

        $judul = "LAPORAN '$xTunaiNonTunai' KEUANGAN FOTOKOPI,PRINT DAN JILID HARIAN ".$rowlokasi->NmLokasi.'<br />'.
    ...
    function getrow($xbulan,$tahun,$xidlokasi,$xTunaiNonTunai){
       ...
       */
       ...
       $xarrayFC = $this->modeltransaksi->getarraystatusplutunainontunai($xbulan,"1",$tahun,$xidlokasi,$xTunaiNonTunai);
       ...
       /****** Print *///
       $xarrayFC = $this->modeltransaksi->getarraystatusplutunainontunai($xbulan,"3",$tahun,$xidlokasi,$xTunaiNonTunai);
       ...
       $xarrayFC = $this->modeltransaksi->getarraystatusplutunainontunai($xbulan,"2",$tahun,$xidlokasi,$xTunaiNonTunai);
       ...
       //******************** Jilid
       $xarrayFC = $this->modeltransaksi->getarraystatusplutunainontunai($xbulan,"4",$tahun,$xidlokasi,$xTunaiNonTunai);
