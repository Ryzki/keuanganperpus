...
class ctrlaprekapfcdinaspribadi extends CI_Controller {
...
//  $this->load->view('test.php');
    ...
    function setDetailFormReport($xidx) {
        ...
        $xBufResult .= setForm('edunitkerja', 'Jenis Transaksi', form_dropdown('edunitkerja', $this->modelunitkerja->getArrayListjenistransaksi(), '0', 'id="edunitkerja" width="150px" onchange="onCbJenisTransksiChange();"')) . '<div class="spacer"></div>';
        ...
        $xBufResult .= '<div class="garis"></div>' . form_button('btSimpan', 'Tampil Data', 'onclick="dotampillaporanrekapdetail(false);"') . form_button('btNew', 'Export Ke Excel', 'onclick="dotampillaporanrekapdetail(true);"') . '<div class="spacer"></div>';
        return $xBufResult;
    }
    ...
    function  dotampillaporan(){
     ...
     $xStatusPLU = $_POST['edidstatusPLU'];
     $this->load->helper('json');
     $this->json_data['data'] =$this->getReport($xBulan,$xTahun,$xJnsTransaksi,$xStatusPLU);
     echo json_encode($this->json_data);
    }

    function getReport($xBulan,$tahun,$xJnsTransaksi,$xStatusPLU) {
        //$xIdEdit = $_POST['edidx'];
        $this->load->helper('form');
        $this->load->helper('common');

        $arrayrow = $this->getrow($xBulan,$tahun,$xJnsTransaksi,$xStatusPLU);
       ...
       $nmbulan =  $array[str_pad($xBulan, 2, '0',STR_PAD_LEFT)];
        ...
        if($xJnsTransaksi =='3'){
            ...
            $rowjudul = $this->modelstatusplu->getDetailstatusplu($xStatusPLU);
         $Judul = $rowjudul->Status;
        } else
        {
            $this->load->model('modeljenistransaksi');
    ...
    function getrow($xbulan,$tahun,$xJnsTransaksi,$xStatusPLU){
       ...
       */
       $this->load->model('modeltransaksi');
       $xQuery = $this->modeltransaksi->getRekapTransaksiStatus($xbulan,$tahun,$xJnsTransaksi,$xStatusPLU);
       $arrayrow[0] = '<td>NO</td>';
      ...
      $TotalSetoran =0;
      if(!empty ($xStatusPLU)){
      $xQuery = $this->modelsetoran->getSumSetoranBulan($xbulan,$tahun,$xStatusPLU);
      foreach ($xQuery->result() as $row) {
            $arrayrow[$i]= '<td> </td>';
            $arrayrow[$i].= '<td> </td>';
